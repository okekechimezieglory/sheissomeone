...
function get(
  ...
  const label = 'Sheissomeone Donate';
  const icon = '/logo.jpeg';
...
async function post(
  ...
  res: NextApiResponse<PostData>
    ...
    // for correct account ordering; - Serialize the transaction and re-create it
    ...
    // Handle potential null signatures
    if (transaction.signature) {
        console.log(base58.encode(transaction.signature)); // Encodes the signature
    } else {
        console.error("Transaction signature is null. The transaction might not be signed properly.");
    }
    ...
    const message = 'Thank you for donating to Sheissomeone';
